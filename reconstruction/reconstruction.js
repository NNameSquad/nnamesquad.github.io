(function () {
  ...
  function updateMole(dtMs) {
    ...
    if (!moleState.popping && !napping && now >= moleState.until)
      pickNextMoleAction(now);
    ...
    if (!moleState.popping && !napping) {
      // integrate position
      moleState.x = clamp(
        moleState.x + moleState.vx * (dtMs / 1000),
        minX,
        maxX,
      );
      moleState.y = clamp(
        moleState.y + moleState.vy * (dtMs / 1000),
        minY,
        maxY,
      );
  ...
  function placeMound() {
    ...
    mound.addEventListener("animationend", () => mound.remove(), {
      once: true,
    });
  ...
  function tick() {
    ...
    if (clockEl)
      clockEl.textContent = `${String(t.h).padStart(2, "0")}:${t.m}:${t.s} ${t.tz}`;
    ...
    if (isNight) {
      ...
      // Rare shooting star
      if (now >= nextShootingEarliest && Math.random() < 0.015) {
        // ~1.5% chance per second
        spawnShootingStar();
